...
class Twitch:
    ...
    def twitch_receive_messages(self):
        ...
        for irc_message in self.receive_and_parse_data():
            ...
            if cmd == 'PRIVMSG':
                ...
                print(f'{irc_message["name"]}: {irc_message["trailing"]}')
            elif cmd == 'PING':
    ...
    def twitch_receive_messages_file(self, output_file):
        privmsgs = []
        for irc_message in self.receive_and_parse_data():
            cmd = irc_message['command']
            if cmd == 'PRIVMSG':
                ...
                with open(output_file, 'a') as file:
                    file.write(f'{irc_message["name"]}: {irc_message["trailing"]}\n')
            elif cmd == 'PING':
                self.sock.send(b'PONG :tmi.twitch.tv\r\n')
            elif cmd == '001':
                print('Successfully logged in. Joining channel %s.' % self.channel)
...
# Create an instance of the Twitch class
twitch = Twitch()

# Connect to Twitch channel
twitch.twitch_connect('xqc')  # Replace 'your_channel_name' with the desired Twitch channel name

# Continuously receive and print Twitch chat messages
while True:
